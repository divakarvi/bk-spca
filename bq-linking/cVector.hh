...
void mult(Matrix& M, cVector& v, cVector& prod);//defns in fourier.cpp
void lusolve(Matrix& A, int *ipiv, cVector& v, Vector& wspace);
void solve(Matrix& A, cVector& v, Vector& wspace);
void ifft(cVector& vf, cVector& v);
//Ar, Ai, Br, Bi are NXN matrices
//ee is an Nx1 complex vector
//evec is 2NxN
//solves the generalized eigenvalue problem with complex matrices
void eig(const Matrix& Ar, const Matrix& Ai, 
	 const Matrix& Br, const Matrix& Bi,
	 cVector& ee, Matrix& evec);
//solves a complex linear system
void solve(const Matrix& Ar, Matrix& Ai,
	   const cVector& b, cVector& soln);
#endif
